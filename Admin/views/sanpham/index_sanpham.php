<div class="container">
    ...
    <table class='table table-bordered'>
        ...
        $sp = new SanPham();
        $limit = 4;
        $page = Utilities::get('page', '1');
        $start = ($page - 1) * $limit;
        $data1 = $sp->allC($start, $limit);
        foreach ($data1 as $r) {
            ?>
            <tr style="text-align: left; font-size: 16px;">
                <td> <?php echo $r['masp']; ?></td>
                <td><img src="<?php echo IMG_SANPHAM . $r['hinhanh']; ?>" alt="" style="width: 100px; height: 100px; object-fit: cover;"><br></td>
    ...
    <div class="container-fruit">
        ...
        $limit = 4;
        $tatca = $sp->tongsp();
        foreach ($tatca[0] as $m => $n){
            $tatca = $n;
        }
        $total_pages = ceil($tatca / $limit);
        $pagLink = "<ul class='pagination'>";
        for ($i = 1; $i <= $total_pages; $i++) {
            $pagLink .= "<li class='page-item'><a class='page-link' href='index.php?controller=sanpham&action=index1&page=" . $i . "'>" . $i . "</a></li>";
        }
        echo $pagLink . "</ul>";
        ?>
    </div>
